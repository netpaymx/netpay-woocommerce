...
class NetPay_Event_Charge_Complete extends NetPay_Event {
	/**
	...
	public function is_resolvable() {
		if ( 'yes' === $this->order->get_meta( 'is_netpay_payment_resolved' ) || $this->is_attempt_limit_exceeded() ) {
			return true;
		}

		$schedule_action = 'netpay_async_webhook_event_handler';
		$schedule_group  = 'netpay_async_webhook';
	...
	public function resolve() {
		...
		$this->order->add_order_note( __( 'NetPay: Received charge.complete webhook event.', 'netpay' ) );
		...
		switch ( $this->data['status'] ) {
			case 'failed':
				...
				$message         = __( 'NetPay: Payment failed.<br/>%s', 'netpay' );
			...
			case 'successful':
				...
				$message = __( 'NetPay: Payment successful.<br/>An amount %1$s %2$s has been paid', 'netpay' );
