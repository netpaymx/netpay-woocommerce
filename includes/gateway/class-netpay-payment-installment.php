...
class NetPay_Payment_Installment extends NetPay_Payment {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'NetPay Installment', 'netpay' );
		$this->method_description = wp_kses(
			__( 'Accept <strong>installment payments</strong> via payment gateway.', 'netpay' ),
			array( 'strong' => array() )
		);
		...
		$this->form_fields = array_merge(
			array(
				'enabled' => array(
					...
					'type'    => 'checkbox',
					'label'   => __( 'Enable NetPay Installment Payments', 'netpay' ),
					'default' => 'no'
				),
	...
	function netpay_installment_form() {
        ...
        if($this->get_option('promotion_products') != null && $this->get_option('promotion')  == 'yes') {
            foreach($items as $item => $values) { 
                $_product_id = (string) $values['data']->get_id();
                if(in_array($_product_id, $this->get_option('promotion_products'))) {
                    $product_msi = true;
                }
			}
            ...
            foreach($items as $item => $values) { 
                ...
                if(!in_array($_product_id, $this->get_option('promotion_products')) && $product_msi) {
                    $apply_msi = false;
        ...
        if($apply_msi === false && $this->get_option('promotion') == 'yes' && $product_msi) {
		...
		if ($this->get_option('promotion') == 'yes' && $apply_msi) {
	...
	public function charge( $order_id, $order ) {
		...
		$netpay_installment_name   = isset( $_POST['netpay_installment_name'] ) ? wc_clean( $_POST['netpay_installment_name'] ) : '';
		...
		$name = explode(" ", $netpay_installment_name);
		...
		&& isset( $netpay_installment_security_code ) ) {
			try {
				$request_token = array(
            		...
            		"cardHolderName" => $netpay_installment_name,
        		...
        		return \NetPay\Api\NetPayCheckout::post(NETPAY_SECRET_KEY, $request_checkout, $installments);
