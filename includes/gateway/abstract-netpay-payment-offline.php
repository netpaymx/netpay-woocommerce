 ...
 * @since 4.0
 */
abstract class NetPay_Payment_Offline extends NetPay_Payment {
	/**
	 * A string of NetPay Source's type
	...
	public function charge( $order_id, $order ) {
		...
		$metadata = array_merge(
			apply_filters( 'netpay_charge_params_metadata', array(), $order ),
			array( 'order_id' => $order_id ) // override order_id as a reference for webhook handlers.
		);

		return NetPayCharge::create( array(
			'amount'      => NetPay_Money::to_subunit( $total, $currency ),
			'currency'    => $currency,
			'description' => apply_filters( 'netpay_charge_params_description', 'WooCommerce Order id ' . $order_id, $order ),
	...
	public function result( $order_id, $order, $charge ) {
		...
		if ( self::STATUS_PENDING === $charge['status'] ) {
			$order->update_status( 'on-hold', sprintf( __( 'NetPay: Awaiting %s to be paid.', 'netpay' ), $this->title ) );
			$order->update_meta_data( 'is_netpay_payment_resolved', 'yes' );
			$order->save();
